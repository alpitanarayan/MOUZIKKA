...
class Player:
    def __init__(self):
        mixer.init()
        self.my_model = Model.Model()                                   #create an object of Model class with name->(my_model)
    ...
    def add_song(self):
        ...
        #if user does not provide any song from song_list --->

        if song_path == "":
            return
        song_name = os.path.basename(song_path)                             #basename will take the path of the file and will return the "filename"
    ...
    def play_song(self):
        mixer.quit()
        mixer.init(frequency=self.audio_tag.info.sample_rate)               #if we will not provide frequecy then this will play with high speed as compare to nirmal speed
    ...
    def load_songs_from_favourites(self):
        ...
        return result,self.my_model.song_dict                              #packing is performed and return tuple
